...
from model.latents import kl_between_laplaces, kl_between_wrapped_cauchy


def physical_prior_anchor(rollout, sigma_physical=0.005, concentration_physical=99.0):
    """Tutorial eq. 27 (professor's L_reg-EB), reduced form: our prior transition MEANS already
    equal the physical dynamics, so KL(p_psi || p_physical) reduces to the scale terms -- anchor
    the learned prior scales toward physical values. Returns mean nats/frame."""
    if rollout.prior_tempo_scales is None:
        return None
    kl_sigma = kl_between_laplaces(torch.zeros_like(rollout.prior_tempo_scales),
                                   rollout.prior_tempo_scales,
                                   torch.zeros_like(rollout.prior_tempo_scales),
                                   torch.full_like(rollout.prior_tempo_scales, sigma_physical))
    kl_conc = kl_between_wrapped_cauchy(torch.zeros_like(rollout.prior_phase_concentrations),
                                        rollout.prior_phase_concentrations,
                                        torch.zeros_like(rollout.prior_phase_concentrations),
                                        torch.full_like(rollout.prior_phase_concentrations,
                                                        concentration_physical))
    return (kl_sigma + kl_conc).mean()
