#!/bin/bash
# CHART on the Beat This frontend (frozen, pretrained final0), resample mode so the
# 50fps logits are upsampled to WaveBeat's 86.13fps and CHART's dynamics/constants
# stay valid. Same CHART variational+PF stack as the WaveBeat run (run_dir1.sh).
# Goal: test whether CHART's trainable bar-pointer tracker recovers continuity (CMLt)
# on Beat This activations that the fixed-lambda DBN could not (SMC oracle CMLt=0.70).
export CUDA_VISIBLE_DEVICES=2
ROOT=/home/sogang/mnt/db_1/jaehoon/beat-tracking/labeled_data
CH=/home/sogang/mnt/db_2/anaconda3/envs/chart/bin/python
cd /home/sogang/jaehoon/CHART
exec $CH -m training.train \
  --mode end2end \
  --extractor beat_this --beat_this_checkpoint final0 --extractor_fps_mode resample --freeze_extractor \
  --dataset_root "$ROOT" \
  --dataset_include ballroom,beatles,hains,rwc_popular \
  --wavebeat_root extractors/wavebeat \
  --seq_len 1024 --batch_size 8 --num_epochs 40 --lr 0.0003 --num_workers 3 \
  --train_length 524288 \
  --examples_per_epoch 1600 --num_meter_classes 8 \
  --kl_anneal_epochs 5 --bce_pos_weight 5.0 --bce_pos_weight_db 15.0 \
  --free_bits_phase 0.2 --free_bits_tempo 0.1 --free_bits_meter 0.2 --max_grad_norm 1.0 \
  --phase_corr_scale 0.1 --tempo_corr_scale 0.15 --decoder_latent_only \
  --tempo_anchor_mode latent --tempo_reversion_alpha 0.4 --taubar_sup_weight 1.0 \
  --meter_sup_weight 1.0 --phase_sup_weight 1.0 \
  --audio_emission --audio_recon_weight 1.0 \
  --scheduled_sampling_max 0.5 \
  --val_every 2 --val_subset_batches 24 --no_wandb \
  --save_ckpt_path checkpoints/ou6_beatthis/chart.pt
