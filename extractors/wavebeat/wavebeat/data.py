...
class DownbeatDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        if len(self.audio_files) == 0 and self.dataset in ["beatles", "rwc_popular"]:
        ...
        for audio_file in self.audio_files:
            ...
            if self.dataset in ["rwc_popular", "beatles"] and filename.endswith(".wav"):
                filename = filename[:-4]
            ...
            if self.dataset == "ballroom":
                ...
                annot_file = os.path.join(self.annot_dir, genre_dir, f"{filename}.txt")
                ...
                if not os.path.isfile(annot_file):
                    stem_tokens = filename.split("_", 1)
                    if len(stem_tokens) == 2 and "." in stem_tokens[1]:
                        genre, idx = stem_tokens[1].split(".", 1)
                        annot_file = os.path.join(self.annot_dir, f"gtzan_{genre}_{idx}.beats")
                if not os.path.isfile(annot_file):
                    annot_file = os.path.join(self.annot_dir, f"{filename}.beats")
                self.annot_files.append(annot_file)
