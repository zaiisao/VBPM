...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--taubar_sup_weight", type=float, default=0.0,
                        help="Opt-in (0=off). Supervises the POSTERIOR tau_bar mean (anchor='latent') to "
                             "the GT clip log-tempo log(2*pi*N_beats/T) so the global-tempo latent carries "
                             "the correct metrical level; the OU reversion (alpha) holds free-running tempo "
                             "there, breaking double-time. One per-clip scalar (milder than tempo_density).")
    parser.add_argument("--meter_sup_weight", type=float, default=0.0,
